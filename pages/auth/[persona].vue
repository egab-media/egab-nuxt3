...
export default defineComponent({
  ...
  head() {
    return {
      ...
      meta: [
        ...
        { hid: 'egab:auth:image:' + this.$route.params.persona, name: 'egab:auth:image:' + this.$route.params.persona, content: '/svg/' + this.$route.params.persona + '.svg' }
      ]
    }
  },
  ...
  methods: {
    handleSwitchAuth() {
      ...
      !this.isRegister
        ? this.$router.push({ name: `auth-persona___${this.$i18n.locale}`, params: { persona: 'journalist' }, query: { action: 'login' } })
        : this.$router.push({ name: `auth-persona___${this.$i18n.locale}`, params: { persona: 'journalist' }, query: { action: 'signup' } })
    }
  }
})
</script>
